...
export default function Header() {
    ...
    const getLinkClass = (href: string) => {
        ...
        return pathname === "/team" || pathname === "/bookacall"
            ? "text-black"
            : isScrolled
            ...
            <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
                <div className="flex lg:flex-1">
                    <Link href="/" className="-m-1.5 flex items-center space-x-2 p-1.5">
                        <img className='h-8 object-contain' src="/logo.png" alt="" />
                    </Link>
                </div>
                ...
                <PopoverGroup className="hidden lg:flex lg:gap-x-12">
                    ...
                    <Link href="/whatwedo" className={ `text-sm font-medium flex items-center uppercase tracking-widest ${getLinkClass("/whatwedo")}` }>What we do <ChevronDownIcon className='h-4 w-4 ml-2' /></Link>
                    <Link href="/resources" className={ `text-sm font-medium uppercase tracking-widest ${getLinkClass("/resources")}` }>Resources</Link>

                </PopoverGroup>
                <div className="hidden lg:flex lg:flex-1 lg:justify-end">
                    <Link href="/bookacall" className={ `text-sm font-medium uppercase tracking-widest rounded-full py-1 px-3 ${textColor} border border-blue-600` }>
                        Book a call &rarr;
                    </Link>
                </div>
            </nav>
